...
- ✅ Единая система авторизации через Supabase Auth
- ✅ Роли пользователей (admin / user)
- ✅ Авторизация через Supabase Auth (для всех)
- ✅ Регистрация с именем и email
- ✅ Страница профиля с вкладками:
  - Мои заявки (просмотр статуса, услуг, суммы)
  - Настройки профиля (редактирование имени, телефона)
- ✅ Кнопка "Войти" / "Профиль" в меню
- ✅ Защищенные роуты с проверкой ролей
- ✅ Фильтрация заявок по email пользователя
...
12. **Компактное статичное меню на всех страницах:**
   ...
   - **Glass effect на Hero:** эффект размытого стекла (glassmorphism) поверх фото - финальный вариант rgba(238, 230, 230, 0.14), blur(2px)
13. **Админка:**
...
15. **Мобильная версия каталога:**
   - **Бейджи фильтров** появляются ПОД свернутым блоком фильтров
   - На десктопе - бейджи внутри карточки фильтров
   - На мобиле когда фильтры закрыты - бейджи под кнопкой "Фильтры"
   - **Поле сортировки** на всю ширину на мобильных (было w-48, стало w-full sm:w-48)
...
- Авторизация: http://localhost:3001/user-auth
- Админка: http://localhost:3001/adminko
...
- [x] Единая система авторизации (реализовано)
- [ ] Email уведомления
...
- Используется Supabase Auth для всех пользователей
- Роль `admin` в `user_metadata` определяет доступ к админке
- API ключ для Google Maps: AIzaSyBFw0Qbyq9zTFTd-tUY6dZWTgaQzuU17R8
- Проект работает на localhost:3001
- См. `ADMIN_SETUP.md` для создания администратора
