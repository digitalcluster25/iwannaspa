...
async function createFinalEmbeddings() {
  try {
    console.log('🚀 Creating final test embeddings...')
    
    // Сначала удаляем существующие эмбеддинги
    ...
    if (deleteError) {
      console.log('ℹ️ No existing embeddings to delete')
    } else {
    ...
    const embeddings = [
      {
        ...
        embedding: Array(1536).fill(0).map((_, i) => (i + 1) * 0.001)
      },
      {
        ...
        embedding: Array(1536).fill(0).map((_, i) => (i + 2) * 0.001)
      },
      {
        ...
        embedding: Array(1536).fill(0).map((_, i) => (i + 3) * 0.001)
      }
    ]
    ...
    const testEmbedding = Array(1536).fill(0).map((_, i) => (i + 1) * 0.001)
